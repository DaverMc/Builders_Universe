...
public class Armorstand116 implements AbstractArmorstand {

    private ArmorStand bukkitArmorstand;

    public Armorstand116(Location location){
        this((ArmorStand) location.getWorld().spawnEntity(location, EntityType.ARMOR_STAND));
    }

    public Armorstand116(ArmorStand bukkitArmorstand){
    ...
    public ItemStack getItem(ArmorstandPart part) {
        EntityEquipment equipment = this.bukkitArmorstand.getEquipment();
        if(equipment == null) return null;
        return switch (part) {
            case HEAD -> equipment.getHelmet();
            case BODY -> equipment.getChestplate();
            //case LEFT_ARM -> equipment.getItemInMainHand();
            //case RIGHT_ARM -> equipment.getItemInOffHand();
            case RIGHT_LEG, LEFT_LEG -> equipment.getLeggings();
    ...
    public void setItem(ArmorstandPart part, ItemStack stack) {
        ...
        switch (part) {
            ...
            case BODY -> equipment.setChestplate(stack);
            //case LEFT_ARM -> equipment.setItemInMainHand(stack);
            //case RIGHT_ARM -> equipment.setItemInOffHand(stack);
            case RIGHT_LEG, LEFT_LEG -> equipment.setLeggings(stack);
            case BOOTS -> equipment.setBoots(stack);
        }
    }
